...
type SpellConfig struct {
	...
	TargetBonusCoefficient float64 // Coefficient applied to target's PseudoStats.SchoolBonusSpellDamage. When 0, BonusCoefficient is used as the fallback.
...
func (unit *Unit) RegisterSpell(config SpellConfig) *Spell {
	...
	spell := &Spell{
		...
		TargetBonusCoefficient: config.TargetBonusCoefficient,
